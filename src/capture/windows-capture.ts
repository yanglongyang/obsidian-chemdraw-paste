...
try { $png=Join-Path $OutDir 'preview.png';$has=[CDP]::Png($png); $x=@('ChemDraw Interchange Format','source.cdx');$b=[CDP]::Read([CDP]::RegisterClipboardFormat($x[0]));if($null -ne $b){$p=Join-Path $OutDir $x[1];[IO.File]::WriteAllBytes($p,$b);$items += @{format=$x[0];file=$x[1];sizeBytes=$b.Length}} } finally {[void][CDP]::CloseClipboard()}
